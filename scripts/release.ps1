...
if (-not $SkipTests) {
    ...
    foreach ($s in $suites) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            # The skill selftest signs off by saying where it put its artifacts,
            # which reads like a problem right after "ok". Step back one line when
            # the tail is just an artifact path.
